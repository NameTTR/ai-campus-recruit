...
class AdminDeploymentGuideControllerTest {
    ...
    void deploymentGuideReturnsStableDefaultVmStartupOrder() throws Exception {
        mockMvc.perform(get("/api/admin/system/deployment-guide"))
                ...
                .andExpect(jsonPath("$.data.steps[0].nodeId").value("vm3"))
                .andExpect(jsonPath("$.data.steps[0].nodeName").value("VM3 Data and AI Node"))
                ...
                .andExpect(jsonPath("$.data.steps[0].verifyUrls[0]").value("tcp://192.168.56.13:3306"))
                .andExpect(jsonPath("$.data.steps[0].verifyUrls[1]").value("tcp://192.168.56.13:6379"))
                .andExpect(jsonPath("$.data.steps[0].verifyUrls[2]")
                        .value("http://192.168.56.13:9000/minio/health/live"))
                .andExpect(jsonPath("$.data.steps[0].verifyUrls[3]").value("tcp://192.168.56.13:9876"))
                ...
                .andExpect(jsonPath("$.data.steps[1].nodeId").value("vm1"))
                .andExpect(jsonPath("$.data.steps[1].verifyUrls[0]").value("http://192.168.56.11:8848/nacos"))
                .andExpect(jsonPath("$.data.steps[1].verifyUrls[1]")
                        .value("http://192.168.56.11:8080/actuator/health"))
                .andExpect(jsonPath("$.data.steps[1].verifyUrls[2]").value("http://192.168.56.11:80/"))
                ...
                .andExpect(jsonPath("$.data.steps[3].nodeId").value("all"))
