...
class AdminDeploymentGuideOverrideControllerTest {
    ...
    void deploymentGuideUsesConfiguredHostsAndPorts() throws Exception {
        mockMvc.perform(get("/api/admin/system/deployment-guide"))
                ...
                .andExpect(jsonPath("$.data.steps[0].verifyUrls[0]").value("http://10.20.1.11:18848/nacos/"))
                .andExpect(jsonPath("$.data.steps[1].verifyUrls[0]").value("tcp://10.20.1.13:13306"))
                .andExpect(jsonPath("$.data.steps[1].verifyUrls[1]").value("tcp://10.20.1.13:16379"))
                .andExpect(jsonPath("$.data.steps[1].verifyUrls[2]")
                        .value("http://10.20.1.13:19000/minio/health/live"))
                .andExpect(jsonPath("$.data.steps[1].verifyUrls[3]").value("tcp://10.20.1.13:19876"))
                ...
                .andExpect(jsonPath("$.data.steps[3].verifyUrls[1]").value("http://10.20.1.11:180/"))
